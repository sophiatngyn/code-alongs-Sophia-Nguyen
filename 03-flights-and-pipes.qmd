...
Flight left earlier than the scheduled departure time.
...
```{r}
#| label: longest-delay
flights_longest_dep_delay <- flights |>
    select(tailnum, carrier, dep_delay) |>
    arrange(desc(dep_delay)) |>
    slice_head(n = 1)

flights_longest_dep_delay
...
Filter the flights data frame for flights heading to RDU or GSO with arrival delay 
less than 0 minutes or departure delay less than 0 minutes. 
...
flights |>
    ...
    relocate(air_time_hr, air_time, distance, mph)
```
...
flights |>
    ...
    mutate(prop = number_of_flights / sum(number_of_flights))
```
...
-   Find mean arrival delay for each month.
